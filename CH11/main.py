...
def naive_knapsack(items, capacity, index, counter):
    ...
    skip_value = naive_knapsack(
        items, capacity, index + 1, counter
    )

    if weight <= capacity:
        take_value = value + naive_knapsack(
...
def dp_knapsack(items, capacity):
    ...
    #       grid[i][j] stores the maximum value possible using the first i items with capacity j.
    ...
    for i in range(1, rows):
        ...
        for j in range(cols):
            if weight <= j:
                grid[i][j] = max(
                    grid[i - 1][j],
                    grid[i - 1][j - weight] + value
                )
            else:
                grid[i][j] = grid[i - 1][j]

    best_value = grid[rows - 1][cols - 1]  # TODO: replace with grid[rows-1][cols-1]
...
def longest_common_substring(a, b):
    ...
    return a[end_position - max_length:end_position]
...
if __name__ == "__main__":
    ...
    print("Substring requires matching letters to be consecutive, while subsequence does not.")
