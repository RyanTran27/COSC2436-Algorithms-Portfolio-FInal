...
def k_nearest(training_set, new_item, k):
    ...
    # The key function computes each item's distance from the new item.
    sorted_items = sorted(
        training_set,
...
def main():
    ...
    for k in [1, 3, 15]:
        ...
        # k=1 relies only on the closest point, so the mislabeled dog
        # causes the wrong prediction. k=15 uses every point, so far-away
        # dogs can influence the result instead of the nearby cat cluster.
    ...
    # The raw weight values are much larger than the quality values, so
    # weight dominates the distance. Normalization puts both features on
    # the same 0-1 scale so both features affect the distance fairly.
    ...
    # Classification uses majority voting, while regression averages the
    # values. The neighbors themselves were found in exactly the same way.
...
#    I would use features such as price, customer rating, distance,
#    cuisine type, and average meal cost.
#
# 2. What would go wrong if one of those features had the exact same
#    value for every restaurant in your dataset?
#    That feature would not help distinguish between restaurants because
#    it has no variation. During normalization, its max and min would also
#    be equal, so it should be handled to avoid division by zero.
# ---------------------------------------------------------------------------
